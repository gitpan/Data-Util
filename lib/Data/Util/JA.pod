...
また，C<"0 but true">は数値として扱われます。
C<Scalar::Util::looks_like_number()>と異なり，
C<"Inf">やC<"NaN">は数値として扱われません。

この関数には検証を行う対応関数がありません。

=item is_integer(value)

I<value>が整数かどうかをチェックします。
また，C<"0 but true">は整数として扱われます。
...
これはC<< do{ no strict 'refs'; \%{$package.'::'} } >>というコードに似ていますが，
