...
C<UNIVERSAL::isa()>を同機能の高速版であるC<fast_isa()>で置き換えます。
この代替ルーチンはPerlディストリビューションのF<universal.t>の
全てのテストを通過するので，C<UNIVERSAL::isa()>の仕様に準じているといえます。

このサブディレクティブはプログラム全体に効果を及ぼします。

=item -fail_handler => \&handler

検証ルーチンが呼び出すエラーハンドラを設定します。

このサブディレクティブは呼び出し元のパッケージにのみ効果を及ぼします。
