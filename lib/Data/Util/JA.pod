...
I<value>が1文字以上の内容を持つ文字列かどうかをチェックします。

C<< is_value($value) && length($value) > 0 >>と同じです。
...
この「数値」とは，C<< sprintf '%g', $value >>などとしたときに警告を出さず，
またPerlプログラム中にリテラルとしておくことができる値という意味です。

すなわち，この関数はC<Scalar::Util::looks_like_number()>と異なり，
C<"Inf">やC<"NaN">を数値として扱いません。また，数値化したときに
警告を出さない例外であるC<"0 but true">>も数値として扱いません。
...
スタッシュ B<stash> と呼ばれるI<invocant>のシンボルテーブルハッシュが
存在すれば，そのスタッシュを返します。
...
I<modified>とI<modifier_type>(C<before>, C<around>, C<after>およびC<original>)
